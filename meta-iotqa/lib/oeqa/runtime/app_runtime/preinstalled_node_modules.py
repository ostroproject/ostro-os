...
class PreinstalledNodeModulesTest(oeRuntimeTest):
    ...
    def all_files_exists(self):
        ...
        :return:
        '''
        for npm_test_file in self.preinstalled_node_modules.values():
            if not os.path.exists(os.path.join(os.path.dirname(__file__),
                                               'files', self.apprt_test_name,
                                               npm_test_file)):
                print('%s not exist!' % \
    ...
    def setUp(self):
        ...
        self.target.run('rm -fr %s' % self.target_npm_dir)
        self.target.run('rm -f %s.tar.gz' % self.target_npm_dir)

        self.files_dir = os.path.join(os.path.dirname(__file__), 'files')
        self.npm_dir = os.path.join(os.path.dirname(__file__),
                                    'files', self.apprt_test_name).rstrip('/')

        if os.path.exists('%s.tar.gz' % self.npm_dir):
            os.remove('%s.tar.gz' % self.npm_dir)

        proc = None
        if self.all_files_exists():
            proc = subprocess.Popen(
                ['tar', '-czf', '%s.tar.gz' % self.npm_dir, self.apprt_test_name],
                cwd=self.files_dir)
            proc.wait()
        else:
            print('There are files not in %s' % self.npm_dir)

        if proc and proc.returncode == 0 and \
            os.path.exists('%s.tar.gz' % self.npm_dir):
            self.target.copy_to(
                os.path.join(
                    os.path.dirname(__file__),
                    'files',
                    '%s.tar.gz' % self.apprt_test_name),
                '%s.tar.gz' % self.target_npm_dir)
            self.target.run('cd /tmp/; ' \
                            'tar -xf %s.tar.gz -C %s/' % (
                                self.target_npm_dir,
                                os.path.dirname(self.target_npm_dir))
                            )
        else:
            print('Fail to compress directory %s ' % self.npm_dir)


    # def test_dummy(self):
    #     self.assertTrue(True, msg = 'This should never be displayed.')
