...
class TestAccelBMA180(oeRuntimeTest):
    """
    @class TestAccelBMA180
    """
    def setUp(self):
        ...
        #shell_cmd("sudo python "+ os.path.dirname(__file__) + "/Connector.py bma180")
        envir = EnvirSetup(self.target)
        envir.envirSetup("bma180","accel")
    def tearDown(self):
        '''unload bma180 driver
        @fn tearDown
        @param self
        @return'''
    ...
    def test_Accel_BMA180(self):
        '''Execute the test app and verify sensor data
        @fn test_Accel_BMA180
        @param self
        @return'''
        print 'start reading data!'
        (status, output) = self.target.run(
                         "chmod 777 /opt/apps/test_accel_bma180.fbp")
        (status, output) = self.target.run(
                         "cd /opt/apps; ./test_accel_bma180.fbp >re.log")
        error = output
        #check whether data collected
        (status, output) = self.target.run(
                         "cp /opt/apps/re.log /home/root/bma180.log")         
