"""
@file test_pressure_tmp006.py
"""
##
# @addtogroup soletta sensor
# @brief This is sensor test based on soletta app
# @brief test pressure function of sensor tmp006 on Galileo/MinnowMax
##
...
class TestPressureTMP006(oeRuntimeTest):
    """
    @class TestPressureTMP006
    """
    def setUp(self):
        ...
        envir.envirSetup("tmp006","pressure")
    ...
    def test_Pressure_TMP006(self):
        '''Execute the test app and verify sensor data
        @fn test_Pressure_TMP006
        @param self
        @return'''
        print 'start reading data!'
        (status, output) = self.target.run(
                         "chmod 777 /opt/apps/test_pressure_tmp006.fbp")
        (status, output) = self.target.run(
                         "cd /opt/apps; ./test_pressure_tmp006.fbp >re.log")
        error = output
        (status, output) = self.target.run(
                         "cp /opt/apps/re.log /home/root/tmp006_pressure.log") 
