"""
@file test_magnet_ak8975.py
"""
##
# @addtogroup soletta sensor
# @brief This is sensor test based on soletta app
# @brief test sensor ak8975 on Galileo/MinnowMax/Edison
##
...
class TestMagnetAK8975(oeRuntimeTest):
    """
    @class TestMagnetAK8975
    """
    def setUp(self):
        ...
        #shell_cmd("sudo python "+ os.path.dirname(__file__) + "/Connector.py ak8975")
        envir = EnvirSetup(self.target)
        envir.envirSetup("ak8975","magnet")

    def tearDown(self):
        '''unload ak8975 driver
        @fn tearDown
        @param self
        @return'''
        ...
        if "Minnow" in output:
           (status, output) = self.target.run(
                         "cd /sys/bus/i2c/devices; \
                          echo 0x0c >i2c-1/delete_device")
        elif "Galileo" in output or "SDS" in output:
           (status, output) = self.target.run(
                         "cd /sys/bus/i2c/devices; \
                          echo 0x0c >i2c-0/delete_device")
        elif "BODEGA" in output:
           (status, output) = self.target.run(
                         "cd /sys/bus/i2c/devices; \
                          echo 0x0c >i2c-6/delete_device")

    def test_Magnet_AK8975(self):
        '''Execute the test app and verify sensor data
        @fn test_Magnet_AK8975
        @param self
        @return'''
        print ('start reading data!')
        (status, output) = self.target.run(
                         "chmod 777 /opt/apps/test_magnet_ak8975.fbp")
        (status, output) = self.target.run(
                         "cd /opt/apps; ./test_magnet_ak8975.fbp >re.log")
        error = output
        (status, output) = self.target.run(
                         "cp /opt/apps/re.log /home/root/ak8975.log") 
