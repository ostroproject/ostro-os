"""
@file test_magnet_lsm303.py
"""
##
# @addtogroup soletta sensor
# @brief This is sensor test based on soletta app
# @brief test sensor lsm303 on Galileo/MinnowMax/Edison
##
...
class TestMagnetLSM303(oeRuntimeTest):
    """
    @class TestMagnetLSM303
    """
    def setUp(self):
        ...
        envir.envirSetup("lsm303","magnet")

    def tearDown(self):
        '''unload lsm303 driver
        @fn tearDown
        @param self
        @return'''
    ...
    def test_Magnet_LSM303(self):
        ...
        (status, output) = self.target.run(
                         "cd /opt/apps; ./test_magnet_lsm303.fbp >re.log")
        error = output
        (status, output) = self.target.run(
