"""
@file test_temperature_mpl3115.py
"""
##
# @addtogroup soletta sensor
# @brief This is sensor test based on soletta app
# @brief test temperature function of sensor mpl3115 on Galileo/MinnowMax
...
class TestTemperatureMPL3115(oeRuntimeTest):
    """
    @class TestTemperatureMPL3115
    """
    def setUp(self):
    ...
    def test_Temperature_MPL3115(self):
        '''Execute the test app and verify sensor data
        @fn test_Temperature_MPL3115
        @param self
        @return'''
        print 'start reading data!'
        (status, output) = self.target.run(
                         "chmod 777 /opt/apps/test_temperature_mpl3115.fbp")
        (status, output) = self.target.run(
                         "cd /opt/apps; ./test_temperature_mpl3115.fbp >re.log")
        error = output
        (status, output) = self.target.run(
                         "cp /opt/apps/re.log /home/root/mpl3115_temperature.log") 
