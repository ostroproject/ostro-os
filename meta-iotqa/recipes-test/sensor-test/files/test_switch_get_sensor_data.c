...
{
	sf_sensor_id_t sensor_id, switched_sensor_id;
	sf_sensor_data_t data;
	unsigned int count;
	int result;
	int i;
	sensor_id = atoi(argv[1]);
        switched_sensor_id = atoi(argv[2]);
	...
	result = sf_connect_sensor(switched_sensor_id);
	if(result < 0)
	{
		fprintf(stdout, "eroor: %d, failed to connect sensor with id %d, already connected to sensor. \n", result, switched_sensor_id);
		return false;
	}
	result = sf_get_sensor_data(switched_sensor_id, &data);
	if(result < 0)
	{
		sf_disconnect_sensor(switched_sensor_id);
		fprintf(stdout, "error: %d, failed to get sensor data with id %d.", result, switched_sensor_id);
		return false;
	}
        for(i = 0; i < data.value_count; i++) {
		fprintf(stdout, "value %d: %f\n", i, data.values[i]);
