...
def get_nodejs_repo(local_nodejs_path, config_file, node_version, store_output = False):
    '''
    ...
    if os.path.exists(local_nodejs_path):
        ...
        get_node_zip_cmd = ['wget', '%s/%s.zip' % (node_archive_url, node_version)]
        ...
        p.communicate()
        if p.returncode == 0:
            p = subprocess.Popen(['unzip', '%s.zip' % node_version], 
                             stdout = subprocess.PIPE, stderr = subprocess.STDOUT,
                             cwd = apprt_files_dir)
            p.communicate()
            if p.returncode == 0:
            	os.rename(os.path.join(apprt_files_dir, 'node-%s' % node_version.lstrip('v')),
            					os.path.join(apprt_files_dir, 'node'))
                return 0

        nodejs_url = config.get('github', 'nodejs_url', 
                                vars = {'nodejs_url': 'https://github.com/joyent/node.git'}
                                )
...
def checkout_nodejs(local_nodejs_path, node_version = 'v0.10.35'):
    ...
    if not os.path.exists(local_nodejs_path):
        sys.stderr.write('node repo directory %s does not exist!\n' % local_nodejs_path)
        return 1
...
class NodejsRuntimeTest(oeRuntimeTest):
    ...
    def setUp(self):
        ...
        self.node_zip = os.path.join(self.files_dir, '%s.zip' % self.target_node_version)
        self.config_path = os.path.join(self.files_dir, 'apprt_nodejs_runtime_config')     

		# Clean the wokrspace directory to avoid error when setUp fails, this ususally
		# causes the failure of second-time to run.
        ...
        sys.stdout.write('Downloading node.js reposiroty...')
        sys.stdout.flush()
        ret = get_nodejs_repo(self.nodejs_dir, self.config_path, self.target_node_version)
        if ret != 0:
        ...
        sys.stdout.write('Checking out node.js to branch %s...' % self.target_node_version)
        sys.stdout.flush()        
        ...
        if ret != 0:
            ...
            sys.exit(1)
        sys.stdout.write('\r' + ' ' * 78 + '\r' )            
        sys.stdout.write('Checking out node.js to branch %s DONE\n' % self.target_node_version)
        sys.stdout.flush()

        sys.stdout.write('Choosing necessary files for tests...')
        sys.stdout.flush()    
        ret = choose_test_files_and_tar(self.nodejs_dir, self.target_node_version)
        if ret != 0:
        ...
        sys.stdout.flush()            
        (status, output) = self.target.copy_to(os.path.join(self.files_dir, 'node_%s_test.tar.gz' % self.target_node_version),
                                '/tmp/node_%s_test.tar.gz' % self.target_node_version
                                )
        if status != 0:
            sys.stderr.write('Fail to copy archives to the target device\n')
            sys.exit(1)
        sys.stdout.write('\r' + ' ' * 78 + '\r' )                   
        sys.stdout.write('Copying node_%s_test.tar.gz to target device DONE\n' % self.target_node_version)
        sys.stdout.flush()
        
        sys.stdout.write('Exracting tar files on target...')
        sys.stdout.flush()               
        (status, output) = self.target.run('tar -xzf  /tmp/node_%s_test.tar.gz -C /tmp/'  % self.target_node_version)
        if status != 0:
            sys.stderr.write('Fail to extract node.js test files\n')
            sys.exit(1)
        sys.stdout.write('\r' + ' ' * 78 + '\r' )               
        sys.stdout.write('Exracting node_%s_test.tar.gz on target device DONE\n' % self.target_node_version)
        sys.stdout.flush()
         
        (status, output) = self.target.run('mkdir -p /tmp/node_%s_test/out/Release' % self.target_node_version)
        if status != 0:
            sys.stderr.write('Fail to mkdir out/Release in node directory\n')
            sys.exit(1)
        sys.stdout.write('mkdir -p /tmp/node_%s_test/out/Release DONE\n' % self.target_node_version)
        sys.stdout.flush()        
             
        (status, output) = self.target.run('ln -s %s /tmp/node_%s_test/out/Release/node' 
                                                       % (self.target_node_path, self.target_node_version))
        if status != 0:
            sys.stderr.write('Fail to link %s -> /tmp/node_%s_test/out/Release/node\n' 
                             % (self.target_node_path, self.target_node_version))
            sys.exit(1)
        sys.stdout.write('ln -s %s /tmp/node_%s_test/out/Release/node DONE\n' 
                                                       % (self.target_node_path, self.target_node_version))
        sys.stdout.flush()   
    ...
    def test_apprt_nodejs_runtime(self):
        ...
        (status, output) = self.target.run('cd /tmp/node_%s_test/; python tools/test.py %s' % (self.target_node_version, test_modules))  

        output = output.strip().split('\r')
        print_test_results(parse_test_cases(output))
        write_test_results(output, start, self.config.get('results', 'formatted_result_file'))
         
#        statistics = output[-1]
#        sys.stdout.write(statistics + '\n')
#        sys.stdout.flush()         
#        statistics_list = statistics.lstrip('[').rstrip(']: Done').replace('%', '').replace('+', '').replace('-', '').split('|')
#        failed_num = int(statistics_list[-1])
#        self.assertEqual(failed_num, 0, msg = 'There are %s test(s) failed!' % failed_num)


    def tearDown(self):
        node_test_dir = os.path.join(self.files_dir, 'node_%s_test' % self.target_node_version)        
    	...
    	if os.path.exists(node_test_dir):
            shutil.rmtree(node_test_dir)
    	    sys.stdout.write('Removing node_%s_test directory DONE\n' % self.target_node_version)
    	    sys.stdout.flush()
    	
    	if os.path.exists('%s.tar.gz' % node_test_dir):
            os.unlink('%s.tar.gz' % node_test_dir)
            sys.stdout.write('Removing node_%s_test.tar.gz DONE\n' % self.target_node_version)
            sys.stdout.flush() 

    	if os.path.exists(self.node_zip):
            os.unlink(self.node_zip)
            sys.stdout.write('Removing node %s.zip DONE\n' % self.target_node_version)
            sys.stdout.flush()

        self.target.run('rm -fr /tmp/node_%s_test/' % self.target_node_version)
        sys.stdout.write('rm -fr /tmp/node_%s_test/ on target DONE\n' % self.target_node_version)
        sys.stdout.flush()
          
        self.target.run('rm -f /tmp/node_%s_test.tar.gz' % self.target_node_version)
        sys.stdout.write('rm -f /tmp/node_%s_test.tar.gz on target DONE\n' % self.target_node_version)
        sys.stdout.flush()
