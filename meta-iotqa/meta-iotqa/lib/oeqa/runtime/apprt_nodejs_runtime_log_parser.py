...
def parse_test_cases(all_test_output):
    '''
    Parse the full node.js API test cases. More information, please refer to the result log
    ...
    for i in xrange(all):
        test_case = dict()
        test_case_info = dict()  
        if all_test_output[start].startswith('[')  and is_a_blank_line(all_test_output[start + 1]):
            ...
            test_case_name = all_test_output[start][release_index + len(': release') + 1:].strip()
...
def write_test_results(output, start_time, log_file):
    ...
    for i in xrange(len(all_tests) - 1):
        tc_name = all_tests[i].keys()[0]
        (min, second) = all_tests[i + 1].values()[0]['last_exec_info'].split('|')[0].lstrip('[').split(':')
        elapse = float(min) * 60 + float(second)
        all_tests[i].values()[0]['result_at'] = time.strftime('%H:%M:%S', time.gmtime(start_time + elapse))
        
    (min, second) = output_seq[-1].split('|')[0].lstrip('[').split(':')
    elapse = float(min) * 60 + float(second)
    all_tests[-1].values()[0]['result_at'] = time.strftime('%H:%M:%S', time.gmtime(start_time + elapse))
    ...
    for t in all_tests:
        ...
        f.write('%s - runexported.py - RESULTS - Testcase %s: %s\n' % (t.values()[0]['result_at'], t.keys()[0], success))
    f.close()
